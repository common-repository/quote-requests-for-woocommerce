 ...
 * @return  mixed|null
 */
function dws_qrwc_get_raw_setting( string $field_id, ?string $group = null ) {
	$group = is_null( $group ) ? 'settings' : Strings::maybe_suffix( $group, '-settings' );
	return dws_qrwc_component( $group )->get_option_value( $field_id );
}
 ...
 * @return  mixed|null
 */
function dws_qrwc_get_validated_setting( string $field_id, ?string $group = null ) {
	$group = is_null( $group ) ? 'settings' : Strings::maybe_suffix( $group, '-settings' );
	return dws_qrwc_component( $group )->get_validated_option_value( $field_id );
}
 ...
 * @return  mixed
 */
function dws_qrwc_validate_setting( $value, string $field_id, ?string $group = null ) {
	$group = is_null( $group ) ? 'settings' : Strings::maybe_suffix( $group, '-settings' );
	return dws_qrwc_component( $group )->validate_option_value( $value, $field_id );
}
